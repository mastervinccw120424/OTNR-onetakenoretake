...
// Audio helper to play sound elements added in index.html
...
function showPage(pageId, skipHistory = false) {
  if (pageId === 'landingPage' && currentUser) {
    pageId = 'courseSelectionPage';
  }
...
function goBack() {
  ...
  showPage(navigationHistory[navigationHistory.length - 1] || (currentUser ? 'courseSelectionPage' : 'landingPage'), true);
}
...
function showResults() {
  ...
  if (percent >= 75) {
    ...
    document.getElementById('resultFeedback').textContent = "You are ready for the board exam! 🎉";
  } else if (percent >= 50) {
    document.getElementById('resultIcon').textContent = "📚";
    document.getElementById('resultTitle').textContent = "Good Job!";
    document.getElementById('resultFeedback').textContent = "Keep practicing to improve!";
  } else {
    document.getElementById('resultIcon').textContent = "💪";
    document.getElementById('resultTitle').textContent = "Time to Review!";
    document.getElementById('resultFeedback').textContent = "More practice will help you improve.";
  }
}
...
function showToast(msg, type = 'info') {
  ...
  // play notification sound for info/success
  if (type === 'info' || type === 'success') playAudio('sound-notification');
}

// Enhance showToast with notification sound
function showToastWithSound(msg, type = 'info') {
  showToast(msg, type);
  if (type === 'info' || type === 'success') playAudio('sound-notification');
}
